# sample3.py
# Demonstrates using lightweight transactions (IF NOT EXISTS) and querying with a secondary index.
...
def main() -> None:
    ...
    try:
        ...
        session.execute(
            "CREATE TABLE IF NOT EXISTS items (id int, sku text, quantity int, PRIMARY KEY (id, sku))"
        )
        session.execute("CREATE INDEX IF NOT EXISTS ON items (sku)")

        session.execute("INSERT INTO items (id, sku, quantity) VALUES (1, 'A1', 100)")
        session.execute("INSERT INTO items (id, sku, quantity) VALUES (2, 'A1', 50)")
        session.execute("INSERT INTO items (id, sku, quantity) VALUES (3, 'B2', 20)")

        print("Items with SKU 'A1':")
        for row in session.execute("SELECT id, quantity FROM items WHERE sku = 'A1'"):
            print(row)

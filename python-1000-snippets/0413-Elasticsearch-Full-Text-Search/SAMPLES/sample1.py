# sample1.py
# Demonstrates indexing documents and running a full-text search query.
...
def main() -> None:
    ...
    while time.time() < deadline:
        try:
            ...
            last_exc = exc
            print("Elasticsearch ping failed; retrying...", exc, flush=True)
            time.sleep(0.5)
    ...
    if es is None or not es.ping():
        ...
        raise RuntimeError(
            ...
            + (f": {last_exc!r}" if last_exc is not None else "")
        )

# sample1.py
# Demonstrate a minimal IMAP server and client interaction using Python's built-in `imaplib`.
...
TEST_EMAIL['Subject'] = 'Test IMAP Message'
TEST_EMAIL.set_content('This is a test message stored in a minimal IMAP server.')
...
class MiniIMAPServer:
    ...
    async def handle_client(self, reader, writer):
        writer.write(b'* OK Mini IMAP4rev1 Service Ready\r\n')
        await writer.drain()
        logged_in = False
        selected = False
        ...
        while not reader.at_eof():
            ...
            if cmd == 'LOGIN':
                ...
                # Always return message 1 when asked for ALL
                writer.write(b'* SEARCH 1\r\n')
                writer.write(f"{tag} OK SEARCH completed\r\n".encode())
            elif cmd == 'FETCH':
                # Only support `FETCH 1 BODY[]`
                if not selected:
...
def run_client():
    ...
    if data[0]:
        ...
        raw = msg_data[0][1]
        print('Fetched message size:', len(raw))
    imap.logout()

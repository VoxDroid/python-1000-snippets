# sample3.py
# Demonstrates parameterized queries and sorting with Neo4j.
...
def main() -> None:
    ...
    try:
        with driver.session() as session:
            session.run("CREATE (m:Movie {title: 'Neo4j in Action', year: 2021})")
            session.run("CREATE (a:Actor {name: 'Neo'})")
            session.run(
                "MATCH (m:Movie {title: 'Neo4j in Action'}), (a:Actor {name: 'Neo'}) "
                "CREATE (a)-[:ACTED_IN]->(m)"
            )

            # Parameterized query
            params = {"actor_name": "Neo"}
            result = session.run(
                "MATCH (a:Actor {name: $actor_name})-[:ACTED_IN]->(m:Movie) "
                "RETURN m.title AS title, m.year AS year ORDER BY m.year DESC",
                params,
            )
            print("Movies acted in by Neo:")

#!/usr/bin/env python3
"""Cloth simulation sample with wind force."""
...
class Cloth:
    ...
    def kinetic_energy(self):
        vel = self.positions - self.prev_positions
        # mass assumed 1.0 for each point
        ke = 0.5 * np.sum(np.linalg.norm(vel, axis=2) ** 2)
        return ke

    def step(self, dt=0.02, gravity=(0.0, 9.8), wind=(0.0, 0.0), damping=0.99, constraint_iters=5):
        current = self.positions
        previous = self.prev_positions
        velocity = (current - previous) * damping
        acceleration = np.array(gravity, dtype=float) + np.array(wind, dtype=float)
...
def main():
    cloth = Cloth(width=7, height=7, spacing=1.0)
    wind = (2.0, 0.0)

    for _ in range(120):
        cloth.step(dt=0.02, wind=wind)

    print(f"Kinetic energy: {cloth.kinetic_energy():.4f}")
